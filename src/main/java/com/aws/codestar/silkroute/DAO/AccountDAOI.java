...
public interface AccountDAOI {
	...
	enum SQL {
		GETACCOUNTBYUSERID("SELECT * FROM TSR_ADMIN.TSR_ACCOUNT WHERE USER_ID=?"),
		ADDMONEYTOACCOUNT("UPDATE TSR_ADMIN.TSR_ACCOUNT SET ACCOUNT_BALANCE = ACCOUNT_BALANCE + ? WHERE USER_ID = ?"),
		TAKEMONEYFROMACCOUNT("UPDATE TSR_ADMIN.TSR_ACCOUNT SET ACCOUNT_BALANCE = ACCOUNT_BALANCE - ? WHERE USER_ID = ? ");
