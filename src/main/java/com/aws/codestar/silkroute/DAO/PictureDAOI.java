...
public interface PictureDAOI { 
    ...
    enum SQL {
        GETALLUSERSPICS("SELECT * FROM TSR_PICTURE WHERE USER_ID=?"),
        UPLOADPHOTOS("INSERT INTO TSR_PICTURES (IMAGE, USER_ID ) values (?,?)"),
        REMOVEPHOTOS("DELETE FROM TSR_PICTURES WHERE PICTURE_ID=?");
