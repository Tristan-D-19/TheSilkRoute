...
public interface UserDAOI { 
    ...
    enum SQL {
		CREATEUSER("INSERT INTO TSR_USER (EMAIL, FIRST_NAME, LAST_NAME, ENCRYTED_PASSWORD, USER_TYPE, ACTIVE ) VALUES (?,?,?,?,0, 1)"),
		CREATEADMINUSER("INSERT INTO TSR_USER (EMAIL, FIRST_NAME, LAST_NAME, ENCRYTED_PASSWORD, USER_TYPE, ACTIVE ) VALUES (?,?,?,?,1, 1)"),
		DELETEUSERBYID("DELETE FROM TSR_USER WHERE USER_ID=?, ACTIVE=0"),
		DEACTIVATEUSER("UPDATE TSR_USER SET ACTIVE =0 WHERE USER_ID=?"), 
		UPDATEUSER("UPDATE TSR_USER SET EMAIL=?, FIRST_NAME=?, LAST_NAME=?, ADDRESS=?,CITY=?, STATE=?, ZIPCODE=?, PHONE NUMBER =?,PROFILE_PIC=?"),
		GETALLUSERS("SELECT EMAIL, FIRST_NAME, ZIPCODE, PROFILE_PIC FROM TSR_USER"),
		ADMINGETALLUSERS("SELECT * FROM TSR_USER"), 
		GETUSERBYID("SELECT * FROM TSR_USER WHERE USER_ID=?"),
		GETUSERBYEMAIL("SELECT * FROM TSR_USER WHERE EMAIL=?, ACTIVE=1"),
		CHANGEUSERPASSWORD("UPDATE TSR_USER SET ENCRYTED_PASSWORD=? WHERE USER_ID=");
