...
public interface ProductPictureDAOI { 
    ...
    enum SQL {
		GETPICTURESBYPRODUCT("SELECT * FROM TSR_ADMIN.TSR_PRODUCT_PICTURE WHERE PRODUCT_ID=?"),
		CREATENEWPRODUCTPICTURE("INSERT INTO TSR_ADMIN.TSR_PRODUCT_PICTURE (PRODUCT_ID, PIC_ID) VALUES (?,?)"),
		REMOVEPRODUCTPICTURE("DELTE FROM TSR_ADMIN.TSR_PRODUCT_PICTURE WHERE PIC_ID=?");
