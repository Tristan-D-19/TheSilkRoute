...
public interface ProductDAOI { 
    ...
    enum SQL {
        GETPRODUCTSBYDEPARTMENT("SELECT * FROM TSR_ADMIN.TSR_PRODUCT WHERE DEPARMENT_ID=?"),
        GETPRODUCTSBYUSER("SELECT * FROM TSR_ADMIN.TSR_PRODUCT WHERE SELLER_ID=?"), 
        SUBTRACTPRODUCTQUANTITY("UPDATE TSR_ADMIN.TSR_PRODUCT SET QUANTITY=QUANTITY-? WHERE PRODUCT_ID=?"),
        ADDPRODUCTQUANTITY("UPDATE TSR_ADMIN.TSR_PRODUCT SET QUANTITY=QUANTITY+? WHERE PRODUCT_ID=?"),
        CREATENEWPRODUCT("INSERT INTO TSR_ADMIN.TSR_PRODUCT (PRODUCT_ID, PRODUCT_NAME, SELLER_ID, DEPARMENT_ID, QUANTITY, PRICE, PRODUCT_DESCRIPTION) VALUES (?,?,?,?,?,?,?)"),
        REMOVEPRODUCT("DELETE FROM TSR_ADMIN.TSR_PRODUCT WHERE PRODUCT_ID=?");
